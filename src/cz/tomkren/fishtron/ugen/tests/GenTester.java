...
public class GenTester {
    ...
    private static void test_ts(int k_max) {



        Checker ch = new Checker();
        Gamma gamma = g_testGamma;
        Type goal = g_testGoal;

        for (int k = 1; k <= k_max; k++) {

            List<TsRes> results = StaticGen.ts(gamma, k, goal, 0);
            BigInteger num = StaticGen.getNum(gamma, k, goal);
            ch.eq(num.intValue(),results.size());

            //Log.it("\n"+k +":"+ results.size());

            for (TsRes res : results) {
                AppTree tree = res.getTree();

                //Log.it(tree.toRawString());
                if (!tree.isStrictlyWellTyped(gamma)) {
                    ch.fail("Not well typed tree generated by ts(..): "+tree);
                }
            }
        }
    ...
    private static void testTreeGenerating(Checker ch, int k, Type t, Gamma gamma, int numSamples, Gen.Opts opts,
        ...
        Log.list(F.map(StaticGen.ts(gamma, k, t, 0), res -> res.getTree().toRawString()));
